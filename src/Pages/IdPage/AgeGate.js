...
export default function AgeGate({
  ...
  if ([day, month, year, country].filter((param) => param).length !== 4) {
    return alert("All fields are required");
  } //gate for blank fields
  ...
  const countryDrinkAge = countries().filter(
    (arrayItem) => arrayItem.code === country
  )[0].age;
  setLegalAge(countryDrinkAge);
  const userAge = new Date(year, month, day).getTime();
  const today = new Date().getTime();
  const ageCheck = Math.floor(
    (today - userAge) / (1000 * 60 * 60 * 24 * 365.25)
    );
  if (countryDrinkAge === null) {
    return setisLegalAge(false);
  } else {
    return countryDrinkAge === 0 || ageCheck >= countryDrinkAge
      ? setisLegalAge(true)
      : setisLegalAge(false);
  } //sends true or false to App.js
}
